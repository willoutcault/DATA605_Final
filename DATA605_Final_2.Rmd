...
Out of the independent variables we picked we see OverallQall with the strongest correlation and all correlations are positive. Next we will test the significance of each correlation significant.
...
```{r}
train2 <- train[sapply(train, is.numeric)]

train2 <- train2[colSums(is.na(train2))<100]
...
```{r}
full.model <- lm(SalePrice ~., data = train2)
